...
| Phase 2G functional journey correction | App/local complete | AI result/photo handoff to editable Marketplace/Lending forms, visible transaction intent/all categories, item-first lending list/map state, Room scan/activity history, and truthful Home/Profile/Recent/Settings logic are implemented locally. The combined debug/release build, 84 JVM tests, lint, XML/20-destination navigation audit, 26 Firebase Emulator Security Rules tests, and secret checks pass without launching an emulator. |
...
| Phase 2G public profiles and Marketplace seller ratings | App/local complete | Reuse Profile with an optional user ID; seller, lender, and conversation avatars open safe public profiles. Marketplace Detail stores one editable 1–5-star rating per reviewer under the seller profile, renders the aggregate on Detail/Profile, and protects identity, listing ownership, score, timestamps, and bounded reads in Firestore Rules. The full local preflight passes; production Rules deployment and two-account device review remain owner checks. |
| Phase 2G sold lifecycle, focused notifications, and local chat time | App/local complete | Owner-only confirmation moves Marketplace `available → sold` as a terminal transition, removes it from browse, blocks new chats, and preserves existing threads. Notifications keeps individual messages in Messages and shows lending lifecycle plus recent sold updates for existing listing-chat participants. Firestore timestamps remain UTC instants and one shared formatter applies the phone's local timezone, locale, and 12/24-hour setting. |
